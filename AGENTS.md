...
- Run benchmark: `cargo run -- -s 10000 -a random -o insertion`
...
- Criterion bench: `cargo bench --bench benchmark` (custom harness, `harness = false`). This runs 180 scenarios (5 sorts × 6 array types × 3 sizes [1000/5000/10000] × puro/com-pre) — expect it to take a long time. There is no way to filter by algorithm/array type from the CLI (all scenarios are hardcoded in `benches/benchmark.rs`).

## Gotchas
- `contar_inversoes` (`src/core/sort/contar_inversoes.rs`) is a brute-force O(n²) double loop. It runs on the full array inside `BenchmarkService::executar_teste`, so keep CLI sizes small (10k is ~10⁸ iterations; 100k+ will crawl). Criterion does NOT use it.
