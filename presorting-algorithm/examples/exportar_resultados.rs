...
const RAIZ_CRITERION: &str = "target/criterion/experimentos_ordenacao";
...
fn ler_cli(arquivo: &str, chave: &str) -> Option<f64> {
    let texto = fs::read_to_string(&format!("{RAIZ_CLI}/{arquivo}")).ok()?;
    for linha in texto.lines() {
        if let Some(valor) = linha.strip_prefix(chave) {
            return valor.trim().split(',').next().and_then(|v| v.parse::<f64>().ok());
        }
    }
    None
}
...
fn principal() {
    ...
    for sort in SORTS {
        for tipo in TIPOS {
            let dir_base = format!("{RAIZ_CRITERION}/{sort}_{tipo}");
            for tamanho in TAMANHOS {
                let mut puro = ler_estimates(&format!("{dir_base}/Tamanho_{tamanho}_Puro"));
                let mut pre = ler_estimates(&format!("{dir_base}/Tamanho_{tamanho}_Com_Pre"));
                let (min_p, q1_p, _, q3_p, max_p) = quartis(&format!("{dir_base}/Tamanho_{tamanho}_Puro"));
                let (min_c, q1_c, _, q3_c, max_c) = quartis(&format!("{dir_base}/Tamanho_{tamanho}_Com_Pre"));

                if let (Some(p), Some(c)) = (puro.take(), pre.take()) {
    ...
    let mut csv_inversoes = String::from("tipo,tamanho,inversoes_iniciais,inversoes_pos_pre,reducao_pct,pre_ns\n");
    let mut csv_pre_custo = String::new();
    let mut precost_por_tamanho: std::collections::HashMap<usize, Vec<f64>> = std::collections::HashMap::new();
    ...
    for tipo in TIPOS {
        for &tamanho in &[1000usize, 5000, 10000, 20000, 100000, 1000000] {
            ...
            let pre = ler_cli(&arquivo, "CSV_PRE_SOZINHO,");
            if let (Some(ini), Some(pos), Some(pre)) = (ini, pos, pre) {
                let reducao = if ini > 0.0 { (ini - pos) / ini * 100.0 } else { 0.0 };
                csv_inversoes.push_str(&format!(
                    "{tipo},{tamanho},{ini:.0},{pos:.0},{reducao:.2},{pre:.0}\n"
                ));
                precost_por_tamanho.entry(tamanho).or_default().push(pre);
                if tamanho == 10000 {
    ...
    for (tamanho, vals) in precost_por_tamanho.iter() {
        ...
        csv_pre_custo.push_str(&format!("{tamanho},{media:.0}\n"));
        precost_pares.push((*tamanho, media));
    }
    precost_pares.sort_by_key(|(tamanho, _)| *tamanho);
    for (tamanho, media) in &precost_pares {
        let por_elem = media / *tamanho as f64;
        linhas_t5.push(format!(
            "{} & {} & {} \\\\",
            mil(*tamanho as f64),
            decimal(media / 1000.0, 2),
            decimal(por_elem, 2)
        ));
    }
    ...
    for sort in SORTS {
        for tipo in TIPOS {
            ...
            if !puro.is_empty() {
                let mp = puro.iter().sum::<f64>() / puro.len() as f64;
                let mc = com.iter().sum::<f64>() / com.len() as f64;
                let mpr = pre.iter().sum::<f64>() / pre.len() as f64;
    ...
    for sort in SORTS {
        ...
        fs::write(format!("{RAIZ_SAIDA}/fig_tempos_{sort}.csv"), csv).unwrap();
    }
    ...
    for (tamanho, media) in &precost_pares {
        csv_fig_precusto.push_str(&format!("{},{:.3}\n", tamanho, media / 1000.0));
    }

    fs::write(format!("{RAIZ_SAIDA}/fig_inversoes.csv"), csv_fig_inversoes).unwrap();
    fs::write(format!("{RAIZ_SAIDA}/fig_ganho.csv"), csv_fig_ganho).unwrap();
    fs::write(format!("{RAIZ_SAIDA}/fig_precusto.csv"), csv_fig_precusto).unwrap();

    fs::write(format!("{RAIZ_SAIDA}/benchmark_consolidado.csv"), csv_bench).unwrap();
    fs::write(format!("{RAIZ_SAIDA}/ganho.csv"), csv_ganho).unwrap();
    fs::write(format!("{RAIZ_SAIDA}/inversoes.csv"), csv_inversoes).unwrap();
    fs::write(format!("{RAIZ_SAIDA}/pre_custo.csv"), csv_pre_custo).unwrap();
    fs::write(format!("{RAIZ_SAIDA}/cli_cross.csv"), csv_cross).unwrap();

    let tabelas = format!(
        r#"%%
%% Tabela 3: redução de inversões (tamanho 10.000)
%%
\begin{{table}}[ht]
\centering
...
"#,
    ...
    fs::write(format!("{RAIZ_SAIDA}/tabelas.tex"), tabelas).unwrap();

    println!("OK: artefatos gravados em {RAIZ_SAIDA}");
    println!("cenários bench: {}", TAMANHOS.len() * SORTS.len() * TIPOS.len());
}
