...
In resource-constrained systems, quadratic sorting algorithms remain relevant due to their low memory footprint, but high inversion counts can degrade their performance. This paper investigates a low-cost \(O(n)\) pre-processing technique based on symmetric swaps and adjacent corrections. Five algorithms were benchmarked across six vector topologies with sizes up to \(10^6\) elements. The technique reduced inversions by up to 100\% at approximately 1,2 ns/element, generating speedups of up to 99.9\% for the adaptive quadratic algorithms (Insertion and Bubble Sort) on structured inputs. For the non-adaptive Selection Sort no significant change was observed, whereas the \(O(n\log n)\) methods did not benefit: Merge Sort showed mixed results and the three-way Quicksort consistently degraded. These results delimit the practical applicability of the technique to adaptive sorting methods.
...
Em sistemas com recursos restritos, algoritmos quadráticos permanecem relevantes pelo baixo consumo de memória, mas altas quantidades de inversões podem degradar seu desempenho. Este trabalho investiga uma técnica de pré-processamento \(O(n)\) baseada em trocas simétricas e correções adjacentes. Foram comparados cinco algoritmos em seis topologias de vetores com tamanhos até \(10^6\) elementos. O método reduziu inversões em até 100\% a aproximadamente 1,2 ns/elemento, gerando ganhos de até 99,9\% para os algoritmos quadráticos adaptativos (Inserção e Bubblesort) em entradas estruturadas. Para o Selection Sort não adaptativo não foi observada mudança significativa, ao passo que os métodos \(O(n\log n)\) não se beneficiaram da técnica: o Merge Sort apresentou resultados mistos e o Quicksort de três vias degradou de forma sistemática. Esses resultados delimitam sua aplicabilidade prática aos métodos de ordenação adaptativos.
...
A Tabela~\ref{tab:tempos} apresenta o tempo médio de execução e o ganho relativo obtido por cada algoritmo sob vetores de 10.000 elementos, evidenciando como a eficiência do pré-processamento é condicionada pela complexidade e adaptabilidade nativas de cada método. Os algoritmos quadráticos sensíveis à ordenação (como \emph{Inserção} e \emph{Bubblesort}) obtiveram os maiores benefícios práticos: no cenário \emph{Zigue-zague}, o pré-processamento reduziu o tempo do \emph{Inserção} de $5.535,18\,\mu s$ para $16,44\,\mu s$ ($+99,7\%$) e do \emph{Bubblesort} de $19.147,42\,\mu s$ para $17,67\,\mu s$ ($+99,9\%$), refletindo a eliminação drástica das inversões. Em contrapartida, métodos de complexidade assintótica $O(n \log n)$, como o \emph{Merge}, apresentaram resultados mistos (com degradação de até 17,8\% e ganhos de até 14,8\%). Como a estrutura de divisão e conquista já garante alta eficiência nativa, o custo computacional da etapa prévia tende a suplantar eventuais ganhos em distribuições de baixa estrutura.
...
A avaliação do tempo total de execução revela uma dicotomia no impacto do pré-processamento: nos algoritmos adaptativos de classe quadrática, notadamente o InsertionSort e o BubbleSort, a drástica eliminação prévia das inversões resulta em ganhos substanciais de desempenho (variando entre $13\%$ e $100\%$), justificados pela matemática do algoritmo — o modesto custo linear da varredura simétrica ($\approx 1{,}2\text{ ns/elemento}$) é vastamente amortizado pela supressão das operações de trocas locais. Em contraste, os métodos de complexidade $O(n\log n)$ apresentam respostas qualitativamente distintas.

O Merge Sort exibe resultados mistos (com degradação de até 17,8\% e ganhos de até 14,8\%): o ganho concentra-se precisamente nas topologias em que o pré-processamento elimina a desordem estrutural (\emph{Invertido}: $+13,1\%$, \emph{Zigue-zague}: $+14,8\%$ em $n=10.000$; ganhos crescentes até $\approx +18\%$ em $n=10^6$), enquanto em \emph{Aleatório}, \emph{Tartarugas} e \emph{Quase ordenado} o overhead da etapa preliminar superpõe-se a um custo de ordenação que já é baixo, resultando em degradação. O Quicksort implementado com particionamento de três vias e mediana de três, por sua vez, apresenta degradação sistemática sob o pré-processamento: leve na maioria das topologias ($-3\%$ a $-5\%$ em \emph{Aleatório}, \emph{Tartarugas}, \emph{Zigue-zague} e \emph{Quase ordenado}), e acentuada em \emph{Duplicados} ($-31,3\%$) e \emph{Invertido} ($-36,8\%$). Esse comportamento é coerente com a natureza não adaptativa dessa implementação: seu custo é dominado pelo número de varreduras de particionamento, que a reestruturação moderada das inversões praticamente não altera. Em \emph{Invertido}, a conversão da entrada em sequência estritamente ordenada remove a alternância de padrões que a seleção de pivô por mediana de três explora, adicionando varreduras sem vantagem alguma em comparações; em \emph{Duplicados}, a reorganização das chaves em blocos adjacentes destroi parcialmente o agrupamento que tornava o particionamento de três vias particularmente eficiente (o cenário puro é o mais rápido de todos: $44,92\,\mu s$). Assim, para a classe $O(n\log n)$, a técnica não se mostra benéfica na implementação avaliada, com raras exceções pontuais (ex.: \emph{Quase ordenado} em $n=10^6$, $+11,1\%$), o que delimita sua aplicabilidade.

O comportamento do Selection Sort confirma sua natureza não adaptativa: como o número de comparações é fixo ($n(n-1)/2$) independentemente da configuração inicial, a redução das inversões não se traduz em ganho algorítmico. De fato, nas medições o algoritmo permaneceu praticamente inalterado na maioria das topologias ($-1,1\%$ a $+3,3\%$), com um aparente decréscimo de $12,3\%$ apenas no cenário \emph{Invertido} em $n=10.000$ (também reproduzido em $n=100.000$, $-12,8\%$). 

Como o pré-processamento transforma a entrada invertida em uma sequência ordenada, a busca do mínimo em cada varredura passa a encontrar o menor elemento já na primeira posição candidata; o ganho esperado seria apenas evitar uma troca por iteração, o que não justifica a degradação observada. A divergência, portanto, parece decorrer de fatores microarquiteturais — especificamente do preditor de desvios (\emph{branch predictor}) da CPU. Em vetores pré-estruturados, a condição central de atualização do índice mínimo torna-se sistematicamente falsa para uma sequência ordenada, gerando um padrão de desvios altamente enviesado que pode acarretar penalidades de \emph{pipeline} distintas das observadas na sequência invertida original. O importante para a análise é que a magnitude dessas variações é pequena (no máximo $\approx 12\%$), não apresentando tendência sistemática de ganho — consistente com a previsão teórica de que um algoritmo não adaptativo não explora a redução de inversões.
...
Toda essa dinâmica — marcada pelo expressivo aproveitamento lógico nos métodos quadráticos adaptativos, pela resposta mista do Merge Sort, pela degradação sistemática do Quicksort de três vias e pela neutralidade do Selection Sort não adaptativo — encontra-se visualmente consolidada na Figura~\ref{fig:tempos}, que detalha o tempo médio de execução puro contraposto ao cenário com pré-processamento para todos os perfis de desordem sob $n=10.000$.
...
Por outro lado, nos algoritmos de complexidade $O(n \log n)$ avaliados, a aplicação do método revelou limitações importantes. Com a implementação de Quicksort utilizada (particionamento de três vias e mediana de três), o pré-processamento causou degradação sistemática de desempenho — de $3\%$ a $5\%$ na maioria das topologias e de $31\%$ a $37\%$ em \emph{Duplicados} e \emph{Invertido} — pois o custo desse método é dominado pelo número de varreduras de particionamento, pouco alterado pela redução de inversões. No Merge Sort os efeitos foram mistos, com ganhos restritos às topologias estruturalmente mais afetadas pela varredura simétrica. Tal constatação indica que a viabilidade prática da técnica deve ser estritamente condicionada a algoritmos quadráticos adaptativos e a entradas com alta desordem estrutural.

Como trabalhos futuros, recomenda-se a realização de testes em ambiente Linux com a infraestrutura \texttt{perf\_event}, viabilizando a análise direta de eventos de hardware, como \emph{cache misses} e \emph{branch mispredictions}. Essa extensão possibilitará investigar detalhadamente os impactos da técnica sobre a hierarquia de memória e a execução especulativa.
