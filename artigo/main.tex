...
No grupo dos algoritmos de complexidade quadrática, com complexidade temporal de $O(n^2)$ no pior caso, foram avaliados o \emph{bubble sort}, o \emph{insertion sort} e o \emph{selection sort}. Os dois primeiros apresentam comportamento adaptativo, uma vez que podem reduzir o número de operações realizadas quando a entrada apresenta algum grau de ordenamento prévio. Essa característica os torna particularmente relevantes para a investigação proposta, pois a redução do número de inversões pode produzir efeitos diretos sobre seu comportamento durante a ordenação. Em contrapartida, o \emph{selection sort} apresenta comportamento essencialmente não adaptativo em relação à disposição inicial dos elementos, realizando, independentemente da configuração da entrada, $\frac{n(n-1)}{2}$ comparações para um vetor de tamanho $n$. Sua inclusão permite, portanto, estabelecer um contraste entre algoritmos potencialmente sensíveis ao pré-processamento e um algoritmo cujo número de comparações permanece invariável.

No grupo dos algoritmos de complexidade quasilinear, com complexidade temporal média de $O(n \log n)$, foram avaliados o \emph{merge sort} e o \emph{quicksort}. A inclusão desses métodos permite verificar se os efeitos observados nos algoritmos quadráticos permanecem relevantes quando o custo assintótico da ordenação é inferior. Dessa forma, o experimento não se restringe a algoritmos diretamente relacionados à contagem de inversões, permitindo analisar também a influência do pré-processamento em métodos cuja estratégia de ordenação apresenta características distintas.
