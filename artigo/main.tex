...
Por outro lado, a análise dos algoritmos de complexidade $O(n \log n)$, como o Quicksort em vetores puramente aleatórios, revela uma ligeira perda de desempenho na abordagem híbrida. Embora o número de inversões seja reduzido pelo pré-processamento, essa redução não necessariamente se traduz em ganho de desempenho quando a ordenação já apresenta comportamento favorável. Uma possível explicação para a perda observada está relacionada ao padrão de acesso à memória introduzido pelo pré-processamento. Ao acessar simultaneamente posições próximas às duas extremidades do vetor, a operação simétrica apresenta um padrão de acesso distinto da varredura sequencial convencional, o que pode reduzir os benefícios de mecanismos de pré-busca de hardware e introduzir custos adicionais de acesso à memória \cite{patterson_computer_2012}. Em algoritmos como o Quicksort, nos quais o ganho proporcionado pela redução de inversões pode ser pequeno em entradas aleatórias, esse custo adicional pode superar os benefícios obtidos pelo pré-processamento.
